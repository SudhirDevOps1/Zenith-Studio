    ...
    <div onClick={() => setSettingsOpen(false)} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-xl bg-[#141524] border border-slate-700 shadow-2xl rounded-2xl overflow-hidden text-slate-200 font-sans flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-slate-800 bg-[#181825]">
          {showResetConfirm ? (
            <div className="flex items-center gap-2 flex-1 mr-2 p-2 bg-rose-950/40 border border-rose-800/60 rounded-lg">
              <span className="text-[10px] text-rose-300 flex-1">⚠️ This will clear all settings <strong>and API keys</strong> from vault. Continue?</span>
              <button
              ...
              <button
                ...
                className="px-2 py-1 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded text-[10px] transition"
              >Cancel</button>
            </div>
          ) : (
            <button
              onClick={() => setShowResetConfirm(true)}
              className="flex items-center gap-1.5 text-slate-400 hover:text-rose-400 text-xs transition"
            >
